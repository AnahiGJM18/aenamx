...
FROM
    ( 
      ...
      IN (
                ...
                'GRANDES',
                'GRUPO CANARIAS',
                'SS.CC. DE AEROPUERTOS',
                'UNIDADES CORPORATIVAS')
      AND  "/BIC/ZHRDIVFN" NOT IN (7000)
    )  
Table__5 
INNER JOIN "_SYS_BIC"."ZAENA/ZCDG_Q6_TRAFICO"  Table__7 ON (
  Table__7."BUS_AREA"   =   Table__5."DIVISION")
   INNER JOIN 
   ( 
      SELECT
            View__26."YEAR",
  ...
  Table__25 ON (
        Table__25."YearMonth"=Table__7."CALMONTH")
WHERE
  (
     Table__5."GRUPO"  
        IN  ( 'GRUPO III','GRUPO I','GRUPO II','GRANDES','GRUPO CANARIAS'  )
        AND
